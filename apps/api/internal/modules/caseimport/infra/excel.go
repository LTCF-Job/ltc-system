...
func (r ExcelAdapter) RenderCaseImportTemplate() ([]byte, error) {
	...
	// 表頭與匯出的「進系統個案個資」逐欄一致，讓匯出檔可以直接回灌。接送車輛兩欄保留版面但暫不使用。
	headers := []string{
	...
	sampleRows := [][]interface{}{
		{"王小明", "一般戶", "D140397675", "男", "032/11/08", "竹南日照據點", "", "", "個管", "林佩宜", "苗栗縣竹南鎮公館里5鄰12號", "苗栗縣竹南鎮大營路123號", "行動不便需輪椅"},
		{"張香香", "低收入戶", "K211281700", "女", "041/03/22", "竹北日照中心", "", "", "照專", "蔡孟儒", "新竹縣竹北市斗崙里8鄰27號", "新竹縣竹北市文興路一段200號", ""},
	}
	...
	_ = f.AddComment(sheetName, excelize.Comment{
		Cell: "A1",
		Text: "＊姓名為必填。生日請填民國 RRR/MM/DD（西元 YYYY/MM/DD 這種寫法也接受）。" +
			"接送車輛(去)/(回) 目前保留欄位但不匯入。" +
			"照護人員會以姓名比對照護人員主檔（同名多筆時才依個管／照專區分）。",
	})
