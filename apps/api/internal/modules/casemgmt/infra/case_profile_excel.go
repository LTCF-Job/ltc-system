...
func (ExcelRenderer) RenderCaseProfileWorkbook(rows []app.CaseProfileRow) ([]byte, error) {
	...
	// A 至 K 嚴格沿用來源工作表的表頭與欄位位置。H 欄的「照護人員」是個管／照專的姓名，
	// 與 A 欄的個案姓名不同義，靠 G 欄「個管or照專」對照角色（匯入端亦同）。
	...
	for i, item := range rows {
		row := []interface{}{
			...
			item.SiteName, item.CareContactRole, item.CareContactName,
