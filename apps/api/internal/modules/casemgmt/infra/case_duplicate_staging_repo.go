...
func (r *CaseDuplicateStagingRepository) Insert(ctx context.Context, cand app.DuplicateCandidate) (uuid.UUID, bool, error) {
	...
	err := db.QueryRow(ctx, `
		INSERT INTO case_import_duplicate_rows (
			...
			care_contact_role, care_contact_name, registered_address, home_address, region,
			service_category, service_usage_type,
			site_id, site_name_raw, outbound_vehicle_id, outbound_vehicle_name_raw,
	...
	`,
		...
		cand.CareContactRole, cand.CareContactName, cand.RegisteredAddress, cand.HomeAddress, cand.Region,
		cand.ServiceCategory, cand.ServiceUsageType,
		cand.SiteID, cand.SiteNameRaw, cand.OutboundVehicleID, cand.OutboundVehicleNameRaw,
...
const duplicateStagingSelectColumns = `
	...
	d.care_contact_role, d.care_contact_name, d.registered_address, d.home_address, d.region,
...
func scanDuplicateCandidate(row pgx.Row) (*app.DuplicateCandidate, error) {
	...
	err := row.Scan(
		...
		&c.CareContactRole, &c.CareContactName, &c.RegisteredAddress, &c.HomeAddress, &c.Region,
