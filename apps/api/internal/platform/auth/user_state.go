...
// CachedUserStateResolver 將已驗證 JWT 的帳號狀態以短 TTL 快取，避免每個 API request
// 都呼叫外部 Admin API；使用者異動時可透過 InvalidateUser 立即清除本機項目。
...
// Validate 先讀取仍有效且角色相符的快取，否則回源查詢並更新快取。
func (c *CachedUserStateResolver) Validate(ctx context.Context, actorID uuid.UUID, role string) (bool, error) {
	if source, ok := c.source.(VersionedUserStateResolver); ok {
		...
		c.mu.RUnlock()
		if cached && entry.role == role && entry.version == version && now.Before(entry.expires) {
	...
	c.mu.RUnlock()
	if ok && entry.role == role && now.Before(entry.expires) {
		return entry.active, nil
	}
