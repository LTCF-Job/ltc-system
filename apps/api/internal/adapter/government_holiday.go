...
func (c *GovernmentHolidayHTTPClient) Fetch(ctx context.Context, year int) ([]repository.HolidayEntity, error) {
	if c == nil || c.Endpoint == "" {
...
func parseHolidayCSV(body []byte, year int) ([]repository.HolidayEntity, error) {
	...
	result := make([]repository.HolidayEntity, 0, len(records)-1)
	for _, record := range records[1:] {
		...
		result = append(result, repository.HolidayEntity{HolidayDate: date, Name: name, Source: "gov_calendar", IsDayOff: isDayOff})
	}
	return result, nil
}
...
func parseHolidayRecords(raw interface{}, year int) ([]repository.HolidayEntity, error) {
	...
	result := make([]repository.HolidayEntity, 0, len(items))
