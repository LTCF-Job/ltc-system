...
type fakeDriverStore struct {
	byID       map[uuid.UUID]*repository.DriverEntity
	createErr  error
	updateErr  error
	assignErr  error
	lastCreate *repository.DriverEntity
	lastUpdate *repository.DriverEntity
	lastAssign *repository.DriverAssignmentEntity
}

func newFakeDriverStore() *fakeDriverStore {
	return &fakeDriverStore{byID: map[uuid.UUID]*repository.DriverEntity{}}
}

func (f *fakeDriverStore) List(ctx context.Context, region, q string, page, pageSize int) ([]repository.DriverEntity, int64, error) {
	return nil, 0, nil
}

func (f *fakeDriverStore) GetByID(ctx context.Context, id uuid.UUID) (*repository.DriverEntity, error) {
	d, ok := f.byID[id]
	if !ok {
...
func (f *fakeDriverStore) Create(ctx context.Context, d *repository.DriverEntity) error {
	f.lastCreate = d
	if f.createErr != nil {
		return f.createErr
	}
...
func (f *fakeDriverStore) Update(ctx context.Context, d *repository.DriverEntity) error {
	f.lastUpdate = d
	if f.updateErr != nil {
		return f.updateErr
	}
...
func (f *fakeDriverStore) AssignVehicle(ctx context.Context, a *repository.DriverAssignmentEntity) error {
	f.lastAssign = a
	return f.assignErr
}
...
func TestDriverService_Update(t *testing.T) {
	...
	t.Run("applies only provided fields", func(t *testing.T) {
		id := uuid.New()
		store := newFakeDriverStore()
		store.byID[id] = &repository.DriverEntity{ID: id, Name: "舊名字", Region: "hsinchu", Status: "active"}
		svc := NewDriverService(store, cfg)
...
func TestDriverService_Reveal(t *testing.T) {
	...
	assert.NoError(t, err)
	id := uuid.New()
	store.byID[id] = &repository.DriverEntity{ID: id, NationalIDCipher: cipher}
