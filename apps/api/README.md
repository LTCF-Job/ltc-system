...
`APP_ENV=production` 時另外強制要求 `SUPABASE_JWKS_URL`、`ALLOWED_ORIGINS`；`TRUSTED_PROXIES` 可留空，空值代表不信任 `X-Forwarded-For`。通知寄信則由 `NOTIFICATION_EMAIL_ENABLED` 明確控制；開啟時才必須提供 `RESEND_API_KEY` 與 `NOTIFY_FROM`。
