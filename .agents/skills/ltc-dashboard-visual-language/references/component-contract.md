...
- 固定 `width` 只給編號、日期、狀態、數值、操作欄；套用上一條 `table-layout="auto"` pattern 的頁面例外，一律改 `min-width` + nowrap class。
