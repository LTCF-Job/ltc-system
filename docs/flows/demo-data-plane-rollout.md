...
- `apps/web/package-lock.json` 有一筆與本次功能無關的 npm 版本 metadata 噪音（安裝依賴時意外產生，4 行 `"dev": true` 標記被移除），worktree 權限限制無法自行 `git checkout` 還原，需要使用者手動執行 `git checkout -- apps/web/package-lock.json` 或確認保留無妨。
