...
├── docs/                    # 本系列文件
├── apps/
│   ├── web/                 # Vue 3 + Vite SPA（部署 Vercel）
│   │   ├── src/api/         # Axios instance 與各資源的 API 函式
│   │   ├── src/views/       # 頁面（對應文件 03 §6 路由表）
│   │   ├── src/components/  # 共用元件
│   │   ├── src/stores/      # Pinia（僅 auth）
│   │   └── src/router/      # Vue Router + 權限 meta
│   └── api/                 # Go（部署 Cloud Run）
│       ├── cmd/server/
│       ├── cmd/exporter/    # Cloud Run Job 進入點
│       ├── internal/
...
├── packages/
│   └── shared-types/        # 前後端共用 TypeScript 型別（由 OpenAPI 產生）
└── .github/workflows/
```
...
| **R-9** | Supabase 免費層專案閒置暫停 | 服務中斷 | 每日排程本身即為心跳；另加 Cloud Monitoring 對 `/healthz` 的可用性檢查 |
...
- **畫面遮罩**：清單頁一律顯示遮罩值，需點擊「顯示完整」才解遮，且該動作寫入存取軌跡。
