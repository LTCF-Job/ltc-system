...
| [backend-api-reference.md](backend-api-reference.md) | 完整 API 路由表（method／path／角色／說明），並標示仍需外部環境確認的整合限制 |
...
| [frontend-permission-logic.md](frontend-permission-logic.md) | 前端權限判斷的實際邏輯：路由守衛判斷順序、模組權限表、個人自訂權限覆蓋規則、角色階層與前後端共用資料來源 |
