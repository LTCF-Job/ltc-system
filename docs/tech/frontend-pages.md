...
依 `apps/web/src/router/index.ts` 的實際路由整理。前端目前沒有 `meta.roles`；`module` 只用來查 effective permission，實際權限判斷見 [frontend-permission-logic.md](frontend-permission-logic.md)。`API` 欄對應到 [backend-api-reference.md](backend-api-reference.md) 的路由分組，方便追一個功能的前後端兩端程式碼。表中的「預設可用角色」只是內建 role 的維護基準，不能取代 backend `RequirePermission` 與 DB permission matrix。
...
| `/settings/users` | `views/settings/UserManagementView.vue` | 具 `settings_users` 權限 | `users/*`（Supabase Admin API；query／pagination／password contract 仍有 review findings） |
| `/settings/roles` | `views/settings/RoleManagementView.vue` | 具 `settings_roles` 權限 | `roles/*`（DB roles；SoD／last-admin contract 仍有 review findings） |
