...
- 車輛歸屬到單位（`vehicles.site_id` → `sites.id`），不再自存區域。API 回傳的 `region` 是由所屬單位 join 出來的唯讀欄位，`GET /vehicles?region=` 也是走單位的區域做模糊比對；未指定單位的車輛 `region` 為空字串，因此不會被任何區域篩選命中。區域自地區主檔下架後改為使用者自由填寫的文字，不再有受控值域。
- 政府「服務車輛清冊」要逐台列出廠牌、車型、出廠年月、三項責任險到期日、前次檢驗日期與是否符合輪椅載運規定，這些欄位由 migration `000010` 加在車輛主檔；車主名稱不另存，就是車輛所屬單位的名稱。
...
1. 當天星期幾落在「個案排班星期」內。單位不再設定開放星期，因此排班只由個案自己的星期設定決定。週日在這裡編碼為 `7`，不是 `0`。
...
司機在匯報表填的姓名常常有全形半形混用、多打空白、簡繁或異體字（例如「淑」vs「俶」），要先正規化才能拿去比對司機主檔：
...
| 欄位 | 名稱 | 原因 |
|---|---|---|
| 19 | C碼必填-復能目標達成情形 | 對應「復能」服務（C 碼），系統只處理交通接送（`BD03`／`DA01`） |
| 20 | C碼必填-復能目標 | 同上 |
| 21 | C碼必填-指導對象 | 同上 |
| 22 | C碼必填-服務內容 | 同上 |
| 23 | C碼必填-指導建議摘要 | 同上 |
| 24 | OT01必填-餐別 | 對應「送餐」服務（`OT01`），系統不處理送餐業務 |

**功能尚未實作**：

| 欄位 | 名稱 | 缺口 |
|---|---|---|
| 12 | 備註 | `ride_records` 沒有對應的自由文字欄位可帶出 |
| 13–16 | 服務人員身分證 2–5 | 一趟只綁定一位司機（見「車輛與司機指派關係」一節），未支援多司機同趟服務 |
| 18 | 訪視未遇 | 排班／趟次資料沒有「訪視未遇」狀態可標記 |
| 27–30 | 出發地／目的地經緯度 | `sites`／個案住家地址只存文字地址，沒有介接地理編碼（geocoding），無法算出經緯度 |

**欄位存在但視情況才有值**（不算缺口，正常業務分支）：

- 第 17 欄「不申報AA09」：只有 `ride_records.not_claimed_aa09 = true` 時才寫入 `1`，其餘為空字串——這是設計上的條件式欄位，不是資料缺漏。
