...
- **自訂角色的 API 存取層級**：已改為 `auth.RequirePermission` 直接查角色的模組權限矩陣（`view`/`edit`/`delete` 三軸），不再是 `viewer`/`staff`/`admin` 三字串白名單，詳見 [role-permission-api-authorization.md](../decisions/role-permission-api-authorization.md)。個人層級的 `customPermissions` 覆蓋也已接上 API 層，現在由 PostgreSQL `auth_user_security_states` 本地投影提供跨 replica 共用的版本來源；Supabase Admin 只負責 identity mutation 與尚未投影舊帳號的首次同步，詳見 [custom-permission-admin-api-enforcement.md](../decisions/custom-permission-admin-api-enforcement.md)。
