...
2. `DriverReportImportView`（`/driver-reports/import`；舊路徑 `/driver-reports/batch-import`、
   ...
   - **批次上傳**頁籤採上下堆疊版面（不用彈窗），整塊限寬 1100px 靠左不撐滿頁面：上方是拖放區，
     下方是主色「選擇檔案」按鈕，再往下是重複上傳提示與每個檔案一列的表格（檔案名稱／車輛／涵蓋
     月份／狀態／說明／操作），即時顯示比對到的車輛／月份與解析狀態。頁面沒有送出按鈕，同一批檔案
     全部解析完就自動匯入。
     ...
     已有資料的月份時，表格上方跳出提示（沿用舊版 Google 表單同步「此月份已同步過」的提醒寫法，
     但說明改為逐筆比對而非覆蓋），自動匯入停在這裡等使用者勾選確認，勾完才續跑，不是匯入後才用
     彈出視窗攔截。
     欄位對應不需使用者當下確認：有系統推薦個案（依姓名相似度與 `[去程]／[回程]` 判斷）的欄位
     自動視為已對應，完全沒有推薦的欄位維持待對應狀態，兩者一起隨 commit 送出。確認匯入時，
     針對同一個檔案推導出的每個月份各自呼叫一次 `?dryRun=false&yearMonth=YYYY-MM`；後端逐列
     比對既有資料，不再整月覆蓋，多數檔案只涵蓋一個月，只有橫跨月份的補傳檔案才會送出多次請求。
     匯入完成後若有欄位進入待維護，或有資料與既有資料衝突，跳出確認視窗詢問是否立即前往
     「待維護資料」頁籤。
...
2. 按「執行前置檢核」打 `POST /exports/precheck`，帶入目前年月、地區與已選取的 `caseIds`。
3. 檢核有阻斷性錯誤（目前只有未裁決混車衝突）就不能匯出，回去處理資料；只有警告（例如個案資料缺漏）時會跳確認框，確認後缺漏欄位留白照樣匯出。
...
資料不完整而在申報檔留白的欄位會以警告列出（`dataGaps`），這份統計只在本次匯出的回應中有值。
...
`CaseListView` 有「匯入 Excel」入口，先 `GET /cases/template` 讓使用者下載範本，填好後上傳走 `POST /cases/import`，後端回傳每列的解析結果（成功／失敗與原因），前端要把失敗列的錯誤原因列出來讓使用者修正後重傳，不是整批全有效才能匯入。單位/車輛比對不到主檔、生日或身分證字號格式錯誤都不擋列，個案照常建立並標記待維護；疑似重複個案則不建立個案，改建立為待裁決暫存列（`GET /cases/import/duplicates`），三者合併在「待維護」頁籤以一列一實體＋彙總問題欄呈現，疑似重複列的「人工裁決」會先呼叫 `POST /cases/import/duplicates/:id/reveal` 解密身分證字號供比對，再呼叫 `POST /cases/import/duplicates/:id/resolve` 確認為新個案或併入既有個案。
