...
- `NOTIFICATION_EMAIL_ENABLED=false`：預設關閉外送；即使環境裡有 `RESEND_API_KEY` 也只寫入資料庫與 log。要真的寄信才改成 `true`，並同時設定 `RESEND_API_KEY` 與已在 Resend 驗證過網域的 `NOTIFY_FROM`
- `RESEND_API_KEY`：只有 `NOTIFICATION_EMAIL_ENABLED=true` 時才需要建立的 Resend API key
...
- 「環境變數」區塊，逐一新增以下八筆（等號後面換成你自己的值）：
  ...
  - `TRUSTED_PROXIES=<實際 ingress proxy 的 IP 或 CIDR>`（選填；直接使用 Cloud Run `run.app` 且沒有固定 proxy 時可省略，不可填全網段）
...
- 「環境變數」加入 `APP_ENV=production`、`SUPABASE_PROJECT_REF=...`、`SUPABASE_JWKS_URL=...`（值跟上面一樣）、`ALLOWED_ORIGINS=https://placeholder.example.com`、`NOTIFICATION_EMAIL_ENABLED=false`。若服務前方有固定 ingress proxy，再加入 `TRUSTED_PROXIES=<實際 ingress proxy 的 IP 或 CIDR>`；直接使用 Cloud Run `run.app` 時可省略。若要外送通知，再設定 `NOTIFY_FROM` 並把 flag 改成 `true`。
...
- [ ] `ltc-api-migrate` 這個工作跟 `ltc-api` 這個服務兩邊都各自設了 `APP_ENV=production`／`SUPABASE_JWKS_URL`／`ALLOWED_ORIGINS`；若前方有固定 ingress proxy，兩邊再設定相同的 `TRUSTED_PROXIES`（拼字也要對，打錯字不會有明確的錯誤提示）
