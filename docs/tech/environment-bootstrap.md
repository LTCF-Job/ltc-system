...
- 「環境變數」區塊，逐一新增以下八筆（等號後面換成你自己的值）：
  ...
  - `TRUSTED_PROXIES=<實際 ingress proxy 的 IP 或 CIDR>`（選填；直接使用 Cloud Run ingress／`run.app` 且沒有固定 proxy 時可留空，不可填全網段）
...
- [ ] `ltc-api-migrate` 這個工作跟 `ltc-api` 這個服務兩邊都各自設了 `APP_ENV=production`／`SUPABASE_JWKS_URL`／`ALLOWED_ORIGINS`（拼字也要對，打錯字不會有明確的錯誤提示）；只有固定 ingress proxy 才需要在 `ltc-api` service 設定 `TRUSTED_PROXIES`
