...
| 類別 | 主要設定 | 用途與注意事項 |
| --- | --- | --- |
| API runtime | `APP_ENV`、`PORT`、`ALLOWED_ORIGINS`、`TRUSTED_PROXIES`、`LOG_LEVEL` | `local` 允許開發降級；production 應 fail closed，`TRUSTED_PROXIES` 留空時不信任 `X-Forwarded-For`，設定時只信任實際 ingress proxy |
