...
| `APP_ENV` | `local` | `production` | `production` 時會強制要求 `SUPABASE_JWKS_URL`、`SUPABASE_URL`（或 `SUPABASE_PROJECT_REF`）、`SUPABASE_SERVICE_ROLE_KEY` 與 `ALLOWED_ORIGINS`，否則拒絕啟動；`TRUSTED_PROXIES` 選填，留空時採 no-proxy 模式 |
...
| `TRUSTED_PROXIES` | `127.0.0.1,::1` | 實際 ingress proxy 的 IP／CIDR 清單 | `production` 可留空，空值不信任 `X-Forwarded-For`；只有固定 ingress proxy 才填其實際來源，禁止填全網段 |
