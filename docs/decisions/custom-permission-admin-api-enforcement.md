...
4. `RequirePermission(resolver, customResolver, module, action)` 簽章新增 `customResolver` 參數：先查角色矩陣，再用 `GetActorID(c)` 查個人覆蓋，兩者用「整個模組物件覆蓋」語意合併（customResolver 有該模組 key 就整包取代角色矩陣的值），語意對齊前端 `apps/web/src/stores/auth.ts` 的 `effectivePermissions`。`cmd/server/routes.go` 全部 89 個 `RequirePermission(perm, "...")` 呼叫點機械式改為 `RequirePermission(perm, customPerm, "...")`（單一模式取代，函式邏輯不因呼叫點而異）。
