...
- 「批次上傳」頁籤採左右分欄版面，不用彈窗：左側固定卡片是拖放上傳區、待處理檔案數量、送出
  按鈕；右側是每個檔案一張卡片的網格，即時顯示比對到的車輛／月份與解析狀態。檔案一加入即自動
  dry run 顯示涵蓋月份；命中已有資料的月份會在左側就地跳出覆蓋警示（沿用舊版 Google 表單同步
  「此月份已同步過」的提醒寫法），需勾選確認風險的核取方塊才能送出，取代原本送出後才彈出
  `ElMessageBox` 攔截的做法。
...
- 未對應欄位的檔案不得被納入確認匯入。放行等於讓那些欄位靜默不寫入搭乘紀錄。
